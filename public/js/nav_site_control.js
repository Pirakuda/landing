...
function сreateNavBtn(btnType, index, actNum, i, pageStructure) {
    ...
    const isActive = actNum === i ? 'page' : 'false';
...
function updateDotsPosition(btnType, curIndex, totalElems) {
	const dotsTrack = returnNavBlock(btnType);
	const maxVisibleDots = btnType === 't' ? 5 : 4;
	...
	updateActiveDot(curIndex, dotsTrack);
}
...
function updateActiveDot(curIndex, dotsTrack) {
	...
	dotsTrack.querySelectorAll('LI').forEach((dotWrap, index) => {
		...
		dotWrap.querySelector('a').setAttribute('aria-current', isActive ? 'page' : 'false');
	});
}
...
function showHidRating(pageStructure) {
	const ratingStr = getCurScrObj(pageStructure).rating ?? 'false';
	const hasRating = ratingStr === 'true';
	ratingContainer.classList.toggle('show', hasRating);
}
...
async function nextLevel(curLevNum, nextLevNum, pageStructure) {
	...
	levelWrap.querySelectorAll('section').forEach(function(level) {
...
function levNavHandler(curLevIndex, newLevNum, pageStructure) {
    ...
    Promise.all([
        ...
        Promise.resolve().then(() => updateDotsPosition('v', newLevNum, pageStructure.levels.length)),
...
async function nextScreen(curScrNum, newScrNum) {
...
function updateScreenMeta(isLevAct, pageStructure) {
	...
	function setMeta(title, slug, description) {
		const baseUrl = pageStructure.type !== 'site'
			? `/webwagen_site/${slug}`
			: `/webwagen_site/${pageStructure.page_slug}/${slug}`;
...
function scrNavHandler(curLevObj, curScrIndex, nextIndex, pageStructure) {
	curLevObj.activeScreen = nextIndex;
	nextScreen(curScrIndex, nextIndex).then(() => {
		const curLevIndex = parseInt(pageStructure.activeLevel);
		const activeLevel = pageStructure.levels[curLevIndex] || 0;
		const totalElems = activeLevel.screens.length;
		updateDotsPosition('h', nextIndex, totalElems);
		queueUpdateScreenMeta(false, pageStructure); // теперь в очереди
	});
}
...
function rightScreenContr(e, pageStructure) {
	...
	if (curScrIndex !== (scrCount - 1)) {
		...
		scrNavHandler(curLevObj, curScrIndex, nextIndex, pageStructure);
	} else {
...
function leftScreenContr(e, pageStructure) {
	...
	console.log('curLevIndex:', curLevIndex, 'curScrIndex:', curScrIndex)
...
// async function loadPageStructure(pageSlug, screenSlug) {
// 	// Валидация входных параметров
// 	if (!pageSlug || !/^[a-zA-Z0-9_-]+$/.test(pageSlug) || 
// 	    (screenSlug && !/^[a-zA-Z0-9_-]+$/.test(screenSlug))) {
// 		console.error("❌ Некорректные параметры:", { pageSlug, screenSlug });
// 		return { success: false, error: 'Invalid pageSlug or screenSlug' };
// 	}
...
// 	try {
// 		const cacheBuster = Date.now(); // для уникальности запроса
// 		const url = `./webwagen_site/api/SiteUploader.php?_=${cacheBuster}`;
...
// 				"Pragma": "no-cache"
// 			},
// 			body: JSON.stringify({ pageSlug, screenSlug }),
...
// 		console.error("❌ Ошибка в loadPageStructure:", error);
...
function setupNavEventListeners() {
	...
	const loadMoreFaqBtn = document.getElementById("load-more-faq-btn");
    const faqContainer = document.getElementById("faq-container");
    const faqPopup = document.getElementById('faq-popup-contr');

	// Обработка кликов (мобильные)
	anchor.forEach(anchor => {
		anchor.addEventListener('click', e => {
			if (deviceType !== 'desktop' || isPortraitOrient()) {
				e.preventDefault();
				const isExpanded = anchor.getAttribute('aria-expanded') === 'true';
				const levelClass = anchor.closest('.nav-level-1') ? 'nav-level-1' : 'nav-level-2';
				closeSiblingSubmenus(anchor, levelClass);
				toggleSubmenu(anchor, !isExpanded);
			} else {
    ...
    anchor.forEach(anchor => {
        ...
        parent.addEventListener('mouseenter', () => {
            if (deviceType !== 'mobile' || isPortraitOrient()) {
	...
	navTopList.querySelectorAll('a.act-anchor').forEach(link => {
		link.addEventListener('click', async e => {
			...
			const navLevel = link.closest('.nav-level-1');
			const mainAnchor = navLevel ? navLevel.children[0] : null;

			if (index && link !== mainAnchor && mainAnchor?.getAttribute('aria-current') === 'page') {
				const curLevIndex = parseInt(pageStructure?.activeLevel ?? 0);
				e.preventDefault();
				stopGuide();

				if (parseInt(index) !== curLevIndex) {
					levNavHandler(curLevIndex, index, pageStructure);
					const submenu = mainAnchor.nextElementSibling;
					if (submenu) submenu.querySelector('li.cur')?.classList.remove('cur');
			...
			// const screenSlug = parts[1] ?? null;
			// if (!pageSlug) {
			// 	updateMsg(popupList, 'Не указан slug страницы');
			// 	return;
			// }

			// const levelWrap = document.getElementById('lev-wrap');
			// if (!levelWrap) {
			// 	updateMsg(popupList, 'Контейнер уровней не найден');
			// 	return;
			// }

			// levelWrap.style.pointerEvents = 'none';

			// try {
			// 	const { success, base, error } = await loadPageStructure(pageSlug, screenSlug);
			// 	if (!success || !base) {
			// 		updateMsg(popupList, error || 'Ошибка загрузки страницы');
			// 		return;
			// 	}

			// 	prevPageStructure = pageStructure;
			// 	pageStructure = base;

			// 	if (!pageStructure.levels || pageStructure.levels.length === 0) {
			// 		updateMsg(popupList, 'Структура страницы пуста');
			// 		return;
			// 	}

			// 	const curLev = levelWrap.querySelector('.level.current');
			// 	if (curLev) {
			// 		curLev.classList.remove('current');
			// 		curLev.classList.add('queue');
			// 	}

			// 	levelWrap.innerHTML = '';
			// 	const newLev = createLev(0, 'level prev', pageStructure);
			// 	levelWrap.appendChild(newLev);
			// 	levelWrap.style.pointerEvents = '';
			// 	queueUpdateScreenMeta(true, pageStructure);
			// } catch (err) {
			// 	updateMsg(popupList, `Ошибка загрузки страницы: ${err.message}`);
			// 	levelWrap.style.pointerEvents = '';
			// }
		});
	});
	...
	document.getElementById('sec-scr-nav-btn-prev').addEventListener('click', function(e) {
		e.preventDefault();
		stopGuide();
		leftScreenContr(e, pageStructure);
	});

	document.getElementById('sec-scr-nav-btn-next').addEventListener('click', function(e) {
		e.preventDefault();
		stopGuide();
		rightScreenContr(e, pageStructure);
	});
...
} /* line 916 - 837 - 804 - 786 - 707 - 728 - 584 - 650 - 458*/

{/* <title><?= htmlspecialchars($scrObj['meta']['title']) ?></title>
<meta name="description" content="<?= htmlspecialchars($scrObj['meta']['description']) ?>">
<meta name="keywords" content="<?= htmlspecialchars($scrObj['meta']['keywords']) ?>">
<meta property="og:title" content="<?= htmlspecialchars($scrObj['meta']['og:title']) ?>">
<meta property="og:description" content="<?= htmlspecialchars($scrObj['meta']['og:description']) ?>">
<meta property="og:image" content="<?= htmlspecialchars($scrObj['meta']['og:image']) ?>"></meta> */}